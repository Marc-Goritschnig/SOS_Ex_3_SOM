...
class ClusteringController(param.Parameterized):
    ...
    def _change_approach(self, ):
        if self.approach == 0:
            self.param['linkage_type'].constant = True
        else:
            self.param['linkage_type'].constant = False
...
class SkyMetaphorController(param.Parameterized):
    ...
    pull_force = param.Number(0.25, bounds=(0, 0.85), label='Pull force')

    def __init__(self, calculate, **params):
        super(SkyMetaphorController, self).__init__(**params)
        self._calculate = calculate

    @param.depends("smooth_factor", "pull_force", watch=True)
    def _change_parameters(self, ):
        self._calculate(self.smooth_factor, self.pull_force)
